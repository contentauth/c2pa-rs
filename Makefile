...
test-wasm-web:
	cd sdk && WASM_BINDGEN_TEST_TIMEOUT=60 CHROMIUM_ARGS="--no-sandbox --disable-dev-shm-usage --disable-gpu --disable-software-rasterizer --disable-extensions --disable-background-networking --disable-sync --metrics-recording-only --disable-default-apps --mute-audio --no-first-run --disable-setuid-sandbox --disable-web-security" CHROMEDRIVER_ARGS="--whitelisted-ips=" wasm-pack test --chrome --headless -- --no-default-features --features="rust_native_crypto, fetch_remote_manifests"
