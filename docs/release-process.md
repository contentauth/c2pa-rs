...
[`experimental-features.yml`](../.github/workflows/experimental-features.yml) builds, lints, and tests the SDK with every `unstable_` feature enabled, on PRs and daily. It is **non-blocking by design** (`continue-on-error`, and not a required check): [experimental features](experimental-features.md) are community-supported and outside the CAI team's maintenance commitments, so `main` is never gated on their health. A persistent failure is grounds for disabling the feature, not for holding up the SDK. The required tier suites (1A/1B/2) already exclude `unstable_` features from their builds; the remaining `--all-features` jobs (`beta-preflight`, the cross-repo canary, and `docs.rs`) are tracked for the same treatment in [#2362](https://github.com/contentauth/c2pa-rs/issues/2362).
