...
## 2. C2PA Archives for Ingredients and Builders
...
builder.add_ingredient_from_stream(
    ...
    .to_string(),
    "application/c2pa",  // Special format for C2PA archives
...
### Use Case 1: Validate Once, Use Many Times
...
### Use Case 2: Work-in-Progress Manifests
...
Both Builders and Readers use shared Context for efficient sharing:
...
### Pattern 2: Edit Existing Content
```rust
builder.set_intent(BuilderIntent::Edit);
builder.add_ingredient_from_stream(
...
### Pattern 6: Link Ingredients to Actions
```rust
...
5. **Store Archives Flexibly** - Files, databases, cloud storage all work
...
A: Anywhere - local files, S3, databases, in-memory.
