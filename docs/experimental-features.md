...
| HTML asset handler (C2PA A.7) | `unstable_html` | Adds `HtmlIO`, associating a Manifest Store with a `text/html` document through a Base64 `<script type="application/c2pa">` element (excluded from the data hash) or a `<link rel="c2pa-manifest">` reference (whole document hashed; the URL is also read back for remote manifest fetching). Implemented in-tree with no new dependencies. | [@mrappard](https://github.com/mrappard) | [#2729](https://github.com/contentauth/c2pa-rs/issues/2729) |
