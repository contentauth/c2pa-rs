...
| OGG Vorbis and Opus manifest embedding | `unstable_ogg` | Adds an `AssetIO` handler for OGG containers (both Vorbis and Opus), embedding the C2PA manifest store as a dedicated logical bitstream per C2PA specification v2.3 section A.3.5, with box hashing per section 18.7.3.7. Registers the `ogg`, `audio/ogg`, `opus`, and `audio/opus` formats. Adds no new crate dependencies. | Harsha Ampar ([@harsha-verda](https://github.com/harsha-verda)) | [#2072](https://github.com/contentauth/c2pa-rs/issues/2072) |
