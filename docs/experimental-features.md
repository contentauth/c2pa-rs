...
| Standard MIDI File asset handler | `unstable_midi` | Adds `MidiIO`, embedding a Manifest Store in Standard MIDI Files (`.mid`, `.midi`) as a Sequencer-Specific Meta Event (`FF 7F`, manufacturer ID `00 43 32`) placed before the End of Track event of a designated `MTrk` chunk. Follows the MIDI embedding section of the C2PA specification draft (not yet in a published version). Implemented in-tree with no new dependencies. | [@mrappard](https://github.com/mrappard) | [#2730](https://github.com/contentauth/c2pa-rs/issues/2730) |
