...
This is an [experimental feature](../experimental-features.md): it is compiled only when the
`unstable_ogg` Cargo feature is enabled, is not part of a default build, and is exempt from the
crate's usual semantic-versioning stability guarantees.

```
cargo build -p c2pa --features unstable_ogg
```

## Implementation summary

- **Module**: `sdk/src/asset_handlers/ogg_io.rs`, gated on `unstable_ogg`.
- **Handler**: `OggIO` with `supported_types()` returning `["ogg", "audio/ogg", "opus", "audio/opus"]`.
- **Traits**: `C2paReader`, `C2paWriter`, `AssetIO`, `AssetPatch`, `AssetBoxHash`.
- **Note**: `RemoteManifestUrl` and `WriteXmp` are not implemented — the C2PA specification does not define XMP or remote reference embedding for OGG containers.
- **Flow**:
...
- `sdk/Cargo.toml` – the `unstable_ogg` feature.
- `sdk/src/asset_handlers/mod.rs` – `pub mod ogg_io`, gated on `unstable_ogg`.
- `docs/experimental-features.md` – registry entry.
- `sdk/src/jumbf_io.rs` – register `OggIO` in `HANDLER_PROTOTYPES`, plus handler tests.
