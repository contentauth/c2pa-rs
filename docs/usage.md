...
The C2PA Rust library requires **Rust version 1.96.0** or newer.
...
The [`sdk/examples`](https://github.com/contentauth/c2pa-rs/tree/main/sdk/examples) directory contains some minimal example code. The [`client/client.rs`](https://github.com/contentauth/c2pa-rs/blob/main/sdk/examples/client/client.rs) file is the most instructive and provides an example of reading the contents of a manifest store, recursively displaying nested manifests.
...
- `openssl`: Uses the vendored `openssl` implementation for cryptography.
...
- `rust_native_crypto`: Uses Rust native cryptography.
