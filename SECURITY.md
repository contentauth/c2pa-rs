...
* **Reproduction that requires calling a private or internal API.** If demonstrating the vulnerability requires code that only compiles as an in-crate test (e.g. `cargo test --lib` against a `#[cfg(test)]` module with `use super::*` insider visibility) to reach items no external consumer's code could ever reference, the report has not shown that someone who can only link against the published artifacts could actually exploit it, regardless of how real the underlying code defect is. It also does not disqualify a report where such access was merely a shortcut for something equally producible without it (e.g. a test fixture buildable just as well with external tooling), provided the private-API step itself could be replaced this way and the actual triggering and observation of the vulnerability goes through the public API or CLI.
