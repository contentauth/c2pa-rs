...
* **Invalid or malicious inputs to API functions other than content likely to be generated off-device.** The primary untrusted input this library is designed to defend against is content that may have been generated off-device, principally C2PA manifests. Other direct inputs to API functions (configuration values, paths, options, and similar parameters supplied by the calling application) are assumed to be trustworthy. It is the responsibility of the application developer to perform reasonable sanity checks on the direct inputs to API functions.
