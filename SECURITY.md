...
* **Use of non-default configuration settings to bypass security requirements** provided the consequences of those settings are sufficiently documented in our [documentation on settings](docs/context-settings.md).
...
* **Invalid or malicious inputs to API functions other than content likely to be generated off-device.** The primary untrusted input this library is designed to defend against is content that may have been generated off-device, principally C2PA manifests. Other direct inputs to API functions (configuration values, paths, options, and similar parameters supplied by the calling application) are assumed to be trustworthy. Similarly, resource stores and working stores are assumed to be trustworthy as they are assumed to have been developed and consumed on the same host system. It is the responsibility of the application developer to perform reasonable sanity checks on the direct inputs to API functions.
* **Reproduction that requires calling a private or internal API.** If demonstrating the vulnerability requires code that only compiles as an in-crate test (e.g. `cargo test --lib` against a `#[cfg(test)]` module with `use super::*` insider visibility) to reach items no external consumer's code could ever reference, the report has not shown that someone who can only link against the published artifacts could actually exploit it, regardless of how real the underlying code defect is. However, this exclusion does not apply merely because a private-API step appeared somewhere in the reproduction. If that step was only a convenience — for instance, using in-crate access to build a test fixture that could equally have been constructed with external tooling — and the vulnerability is still triggered and observed entirely through the public API or CLI, the report remains eligible. To rely on this exception, the report must explicitly describe how the code path would be reached via the public API or CLI — an assertion that a public path "probably" exists, without describing it, is not sufficient.
