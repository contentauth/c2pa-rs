...
* **Denial-of-service attacks are generally not accepted.** As a specific example, out-of-memory conditions caused by sending large input data are not accepted. We view it as the responsibility of the host application to enforce size limits on raw input data based on configuration information specific to the host. There is an exception for scenarios where a “reasonably small” input generates an unreasonably large memory allocation. For example, if an input file of 1MB would cause a 200MB memory allocation (i.e. the c2pa-rs allocation were a very large multiple of the original input), that ticket will generally be accepted.
...
The following characteristics represent issues that we know are problematic. Work is planned or in progress to address these issues. Tickets for such issues should be **rejected** until the work is complete and published.
...
* **Parsing errors that cause crashes or undefined behavior** – unless as described above under "denial of service."
* **Failure to detect invalid / untrusted status in C2PA manifests** – unless as described above under “use of non-default configuration settings” or documented above as “temporarily unwelcome.”
* **Default settings that create vulnerabilities** – unless documented above as “temporarily unwelcome.”
