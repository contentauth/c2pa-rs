...
// Generate TimeStamp based on rfc3161 using "data" as MessageImprint and return
// raw TimeStampRsp bytes
        ...
        for signer_info in sd.signer_infos.iter() {
            ...
            let cert = match certs.iter().find_map(|cc| {
                ...
                match &signer_info.sid {
                    crate::internal::asn1::rfc5652::SignerIdentifier::IssuerAndSerialNumber(sn) => {
                    ...
                    crate::internal::asn1::rfc5652::SignerIdentifier::SubjectKeyIdentifier(ski) => {
            ...
            if let Some(attributes) = &signer_info.signed_attributes {
                    ...
                    })
                {
                    ...
                    // check to see if time string has been modified todo: when is this an error
                    // case
