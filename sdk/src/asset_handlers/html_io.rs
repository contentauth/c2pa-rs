...
//! A.7, using the [`c2pa-text`](https://crates.io/crates/c2pa-text) crate. The
//! Manifest Store (JUMBF) is Base64-encoded into a
//! `<script type="application/c2pa">` element placed in the document `<head>`.
//! External `<link rel="c2pa-manifest">` references are recognized on read but
//! are treated as an external (non-embedded) store.
//!
//! This handler is gated behind the non-default `html` feature and is
//! maintained by EncypherAI. Please report issues at
//! <https://github.com/encypherai/c2pa-text/issues>.

use std::{fs::File, path::Path};

use c2pa_text::html::{build_html_script, embed_html_inline, extract_html, HtmlMethod};
...
impl CAIWriter for HtmlIO {
    fn write_cai(
        ...
        // An existing manifest element is replaced in place so it keeps its
        // original position; update manifests require this. Only a document
        // with no C2PA element yet gets a fresh insertion into <head>.
        let signed = match find_script_span(&text) {
            Some((offset, length)) => {
                let element = build_html_script(store_bytes);
                let mut out = String::with_capacity(text.len() - length + element.len());
            ...
            None => {
                embed_html_inline(&text, store_bytes, "\n")
...
pub mod tests {
    ...
    /// Replacing a manifest keeps the `<script>` element at its original
    /// position, as required for update manifests.
    #[test]
    fn html_io_write_preserves_element_position() {
        let jumbf: Vec<u8> = vec![0, 0, 0, 8, b'j', b'u', b'm', b'b'];
        let html_io = HtmlIO::new("html");
        ...
        let signed_text = String::from_utf8(signed.get_ref().clone()).unwrap();
        let (original_offset, _) = find_script_span(&signed_text).expect("span");

        // Replace with a larger manifest.
        ...
        let rewritten = String::from_utf8(signed2.get_ref().clone()).unwrap();
        let (new_offset, _) = find_script_span(&rewritten).expect("span after rewrite");
