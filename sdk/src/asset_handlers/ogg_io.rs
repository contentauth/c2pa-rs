...
//! <div class="warning">
//!
//! **Experimental.** This module is available only with the `unstable_ogg` feature
//! enabled. It is exempt from this crate's usual semantic-versioning stability
//! guarantees and may change in a backward-incompatible way, or be removed entirely, in
//! any release. See [Experimental features
//! policy](https://github.com/contentauth/c2pa-rs/blob/main/docs/experimental-features.md).
//!
//! </div>
//!
...
use crate::{
    asset_io::{
        AssetBoxHash, AssetIO, AssetPatch, BoxMap, C2paReader, C2paWriter, ObjectLocations,
        ObjectType, ReadSeek, ReadWriteSeek, C2PA_BOXHASH,
    },
    error::{Error, Result},
};
...
fn read_page(reader: &mut dyn ReadSeek) -> Result<Option<OggPage>> {
    ...
    if &capture != OGG_CAPTURE {
        return Err(Error::InvalidAsset(
            "not a valid OGG file: missing OggS capture pattern".to_string(),
        ));
    }
    ...
    if computed_crc != stored_crc {
        return Err(Error::InvalidAsset(format!(
            "OGG CRC mismatch on page at offset {file_offset}"
        )));
    }
...
fn read_all_pages(reader: &mut dyn ReadSeek) -> Result<Vec<OggPage>> {
    reader.rewind()?;
...
impl C2paWriter for OggIO {
    fn write_c2pa(
        ...
        let c2pa_serial: Option<u32> = pages
            .iter()
        ...
        let bos_pages: Vec<&OggPage> = non_c2pa_pages
            .iter()
            .filter(|p| p.is_bos())
            .copied()
            .collect();
        let data_pages: Vec<&OggPage> = non_c2pa_pages
            .iter()
            .filter(|p| !p.is_bos())
            .copied()
            .collect();
    ...
    fn get_object_locations(
        ...
        if has_c2pa {
            ...
            input_stream.rewind()?;
            self.write_c2pa(input_stream, &mut temp_output, &[1, 2, 3, 4])?;
        }
        ...
        let mut regions: Vec<ObjectLocations> = Vec::new();
...
impl AssetPatch for OggIO {
    fn patch_c2pa(&self, stream: &mut dyn ReadWriteSeek, store_bytes: &[u8]) -> Result<()> {
        stream.rewind()?;
        let pages = read_all_pages(stream)?;
        ...
        for (old_page, new_page) in c2pa_pages.iter().zip(new_pages.iter()) {
            ...
            stream
                .seek(SeekFrom::Start(old_page.file_offset))
                .map_err(Error::IoError)?;
            stream.write_all(&serialized).map_err(Error::IoError)?;
        }
    ...
    fn get_box_map(&self, input_stream: &mut dyn ReadSeek) -> Result<Vec<BoxMap>> {
        ...
        for page in &pages {
            ...
            if extend {
                ...
                let mut box_map = BoxMap::new(vec![name], offset, page_size);
                box_map.excluded = excluded;
...
mod tests {
    ...
    fn test_read_cai_no_c2pa() {
        ...
        let mut cursor = Cursor::new(SAMPLE_OGG);
        match handler.read_c2pa(&mut cursor) {
    ...
    fn test_read_cai_too_short() {
        ...
        match handler.read_c2pa(&mut cursor) {
            Err(Error::InvalidAsset(_)) => {}
            other => panic!(
                "expected InvalidCapture for truncated stream, got {:?}",
                other
            ),
        }
    }
    ...
    fn test_read_cai_too_many_manifests() {
        ...
        handler
            .write_c2pa(&mut input, &mut signed, TEST_MANIFEST)
            .unwrap();
    ...
    fn test_write_read_roundtrip() {
        ...
        handler
            .write_c2pa(&mut input, &mut output, TEST_MANIFEST)
            .expect("write_c2pa failed");

        // Read it back.
        let read_back = handler
            .read_c2pa(&mut output)
            .expect("read_c2pa after write failed");
    ...
    fn test_write_read_roundtrip_opus() {
        ...
        handler
            .write_c2pa(&mut input, &mut output, TEST_MANIFEST)
            .expect("write_c2pa failed for Opus");

        let read_back = handler
            .read_c2pa(&mut output)
            .expect("read_c2pa after write failed for Opus");

        assert_eq!(
            read_back, TEST_MANIFEST,
            "Opus manifest round-trip mismatch"
        );
    }
    ...
    fn test_write_read_roundtrip_large_manifest() {
        ...
        handler
            ...
            .expect("write_c2pa failed for large manifest");

        let read_back = handler
            .read_c2pa(&mut output)
            .expect("read_c2pa failed for large manifest");
    ...
    fn test_write_read_roundtrip_255_boundary() {
        ...
        handler
            .write_c2pa(&mut input, &mut output, &manifest)
            .expect("write_c2pa failed for 255-boundary manifest");

        let read_back = handler
            .read_c2pa(&mut output)
            .expect("read_c2pa failed for 255-boundary manifest");

        assert_eq!(
            read_back, manifest,
            "255-boundary manifest round-trip mismatch"
        );
    }
    ...
    fn test_write_replaces_existing() {
        ...
        handler
            .write_c2pa(&mut input, &mut output1, b"first-manifest")
            .unwrap();

        // Write again over the result.
        let mut output2 = Cursor::new(Vec::new());
        handler
            .write_c2pa(&mut output1, &mut output2, b"second-manifest")
            .unwrap();

        let read_back = handler.read_c2pa(&mut output2).unwrap();
    ...
    fn test_remove_manifest() {
        ...
        handler
            .write_c2pa(&mut input, &mut with_manifest, TEST_MANIFEST)
            .unwrap();
        ...
        match handler.read_c2pa(&mut removed) {
            Err(Error::JumbfNotFound) => {}
    ...
    fn test_audio_preserved_after_write_remove() {
        ...
        handler.remove_c2pa(&mut signed, &mut restored).unwrap();
    ...
    fn test_write_empty_removes() {
        ...
        let mut output = Cursor::new(Vec::new());
        handler
            .write_c2pa(&mut with_manifest, &mut output, &[])
            .unwrap();

        match handler.read_c2pa(&mut output) {
            Err(Error::JumbfNotFound) => {}
    ...
    fn test_patch_same_size() {
        ...
        handler.patch_c2pa_file(&temp_path, new_manifest).unwrap();
    ...
    fn test_patch_size_mismatch() {
        ...
        let result = handler.patch_c2pa_file(&temp_path, b"short");
    ...
    fn test_file_roundtrip() {
        ...
        handler.save_c2pa_store(&temp_path, TEST_MANIFEST).unwrap();
    ...
    fn test_file_remove() {
        ...
        fs::write(&temp_path, SAMPLE_OGG).unwrap();
        handler.save_c2pa_store(&temp_path, TEST_MANIFEST).unwrap();
        handler.remove_c2pa_store(&temp_path).unwrap();
