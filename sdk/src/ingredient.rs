...
impl Ingredient {
    ...
    pub(crate) fn add_to_claim(
        ...
        if let Some(thumb_ref) = self.thumbnail_ref() {
            // If the identifier is a JUMBF URI, extract the manifest label from it.
            // If it is a local resource ID (e.g. generated by add_thumbnails), fall back to
            // the ingredient's own active manifest label so we still suppress the thumbnail
            // when that manifest's thumbnails are being redacted.
            let thumbnail_is_redacted =
                jumbf::labels::manifest_label_from_uri(&thumb_ref.identifier)
                    .as_deref()
                    .or(self.active_manifest.as_deref())
