...
pub struct Builder {
    ...
    // Optional watermark to add if supported
...
impl Builder {
    ...
    pub fn set_watermark<S: Into<String>>(&mut self, url: S) -> &mut Self {
        self.watermark = Some(url.into());
        self
    }
    ...
    fn to_claim(&self) -> Result<Claim> {
        ...
        claim.set_watermark("test"); // just set a watermark for hackweek, value autogenerated
        
        if let Some(watermark) = &self.watermark {
            claim.set_watermark(watermark);
        }
