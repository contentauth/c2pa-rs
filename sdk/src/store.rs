...
pub mod tests {
    ...
    */
    ...
    //     let (format, mut input_stream, mut output_stream) = create_test_streams("CA.jpg");
    ...
    //     let thumb_absolute_path = crate::jumbf::labels::to_absolute_uri(pc.label(), &thumb_path);

    //     // add prior store as ingredient to create a new store
    //     input_stream.rewind().unwrap();
    //     let ingredient_bytes = load_jumbf_from_stream(format, &mut input_stream).unwrap();
    //     Store::load_ingredient_to_claim(&mut claim, &ingredient_bytes, ).unwrap();
