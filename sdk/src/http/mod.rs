...
/// Both HTTP resolvers bundled together.
///
/// Use this as a single parameter in functions that need to support both sync
/// and async HTTP paths (e.g. via the `async_generic` pattern), so callers
/// pass one value instead of two.
pub trait HttpResolvers: MaybeSend + MaybeSync {
    fn sync_resolver(&self) -> Arc<dyn SyncHttpResolver>;
    fn async_resolver(&self) -> Arc<dyn AsyncHttpResolver>;
}

impl<T: HttpResolvers + ?Sized> HttpResolvers for &T {
    fn sync_resolver(&self) -> Arc<dyn SyncHttpResolver> {
        (**self).sync_resolver()
    }
...
impl<T: HttpResolvers + ?Sized> HttpResolvers for Arc<T> {
    fn sync_resolver(&self) -> Arc<dyn SyncHttpResolver> {
        (**self).sync_resolver()
    }
...
/// Concrete [`HttpResolvers`] implementation that holds both resolvers independently.
///
/// Use this when you have access to an implementation of HttpResolvers and
/// have access to individual sync and async resolvers
/// For example: Context implements HttpResolvers. So, no need to use ResolverBundle
/// Signer/TimeStampProvider do not have access to Context, but have access to individual
/// resolvers. So, they can use this implementation.
///
...
impl ResolverBundle {
    pub fn new(
        sync_resolver: impl SyncHttpResolver + 'static,
        async_resolver: impl AsyncHttpResolver + 'static,
    ) -> Self {
        Self {
            sync_resolver: Arc::new(sync_resolver),
            async_resolver: Arc::new(async_resolver),
        }
    }
}

impl HttpResolvers for ResolverBundle {
    fn sync_resolver(&self) -> Arc<dyn SyncHttpResolver> {
        self.sync_resolver.clone()
    }

    fn async_resolver(&self) -> Arc<dyn AsyncHttpResolver> {
        self.async_resolver.clone()
    }
}
