...
mod tests {
    ...
    async fn test_connected_identities_valid() {
        ...
        // The proxy above points did:web resolution at a loopback mock server. The default
        // `network_security` policy allows direct requests to internal hosts (it only blocks
        // redirect following), so no allow-list is required here.
        #[cfg(not(target_arch = "wasm32"))]
