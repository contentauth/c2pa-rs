...
/// Public entry point — uses the default HTTP resolver for timestamp requests.
...
pub fn sign(
    ...
    if _sync {
        ...
        sign_with_resolver(signer, data, box_size, tss, &resolver)
    } else {
        let resolver = AsyncGenericResolver::with_redirects().unwrap_or_default();
        sign_with_resolver_async(signer, data, box_size, tss, &resolver).await
    }
}

/// Internal entry point that accepts a caller-provided HTTP resolver for timestamp requests.
#[async_generic(async_signature(
    signer: &dyn AsyncRawSigner,
    data: &[u8],
    box_size: Option<usize>,
...
pub(crate) fn sign_with_resolver(
    signer: &dyn RawSigner,
    data: &[u8],
    box_size: Option<usize>,
    tss: TimeStampStorage,
    ...
    if _sync {
        match tss {
            TimeStampStorage::V1_sigTst => sign_v1(signer, data, box_size, tss, resolver),
            TimeStampStorage::V2_sigTst2_CTT => sign_v2(signer, data, box_size, tss, resolver),
        }
    } else {
        match tss {
            TimeStampStorage::V1_sigTst => sign_v1_async(signer, data, box_size, tss, resolver).await,
            TimeStampStorage::V2_sigTst2_CTT => sign_v2_async(signer, data, box_size, tss, resolver).await,
        }
    }
}
...
pub fn sign_v1(
    ...
    let unprotected_header = if _sync {
        build_unprotected_header(signer, data, &protected_header, tss, resolver)?
    } else {
        build_unprotected_header_async(signer, data, &protected_header, tss, resolver).await?
    };
...
pub fn sign_v2(
    ...
    if _sync {
        sign_v2_embedded(signer, data, box_size, CosePayload::Detached, None, tss, resolver)
    } else {
        sign_v2_embedded_async(signer, data, box_size, CosePayload::Detached, None, tss, resolver).await
    }
}
...
pub fn sign_v2_embedded(
    ...
    let unprotected_header = if _sync {
        build_unprotected_header(signer, &sig_data_cbor, &protected_header, tss, resolver)?
    } else {
        build_unprotected_header_async(signer, &sig_data_cbor, &protected_header, tss, resolver).await?
    };
...
#[async_generic(async_signature(signer: &dyn AsyncRawSigner, data: &[u8], p_header: &ProtectedHeader, tss: TimeStampStorage, resolver: &dyn AsyncHttpResolver,))]
fn build_unprotected_header(
    ...
    let mut unprotected_h = if _sync {
        add_sigtst_header(signer, data, p_header, unprotected_h, tss, resolver)?
    } else {
        add_sigtst_header_async(signer, data, p_header, unprotected_h, tss, resolver).await?
    };
