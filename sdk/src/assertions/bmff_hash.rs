...
impl BmffHash {
    ...
    pub fn verify_in_memory_hash(
        ...
        self.verify_in_memory_hash_with_progress(
            data,
            alg,
            &mut |_, _| Ok(()),
            default_hash_buffer_size(),
    ...
    pub fn verify_hash(
        ...
        self.verify_hash_with_progress(
            asset_path,
            alg,
            &mut |_, _| Ok(()),
            default_hash_buffer_size(),
    ...
    */
    ...
    /// [`hash_stream_by_alg_with_progress`] so it fires once per `max_hash_buffer_size_in_bytes`-sized range,
    /// giving sub-pass granularity for large files.  For Merkle paths it fires once per hash
    /// operation.  `total` is always `0` (indeterminate) from this layer because the pass count
    /// is not known until the file structure is parsed.
        ...
        F: FnMut(u32, u32) -> crate::error::Result<()>,
    {
        ...
        // (one tick per read range, `max_hash_buffer_size_in_bytes` bytes max)
        // instead of a single pre-hash tick.
