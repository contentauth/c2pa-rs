...
fn test_validation_results_schema_compliance() -> Result<()> {
    ...
    if let Some(validation_info) = json_value.get("validationInfo") {
        ...
        if let Some(signature) = vi.get("signature") {
            assert!(signature.is_array(), "validationInfo.signature should be array of status codes");
            for item in signature.as_array().unwrap() {
                assert!(item.is_string(), "validationInfo.signature items should be strings");
            }
        }
    ...
    if let Some(first) = manifests.first() {
        ...
        if let Some(deltas) = first.get("ingredientDeltas") {
            ...
            for item in deltas.as_array().unwrap() {
                ...
                assert!(obj.contains_key("ingredientAssertionURI"), "Delta should have ingredientAssertionURI");
...
fn test_manifest_validation_and_status_schema_compliance() -> Result<()> {
    ...
    if let Some(validation_info) = json_value.get("validationInfo") {
        ...
        if let Some(signature) = info_obj.get("signature") {
            assert!(signature.is_array(), "signature should be array of status codes");
            for item in signature.as_array().unwrap() {
...
fn test_cr_json_schema_file_valid_and_matches_format() -> Result<()> {
    ...
    let definitions = schema_value.get("definitions").and_then(|d| d.as_object()).expect("schema must have definitions");
    let manifest_def = definitions.get("manifest").and_then(|m| m.as_object()).expect("schema must define manifest");
    let manifest_props = manifest_def.get("properties").and_then(|p| p.as_object()).expect("manifest must have properties");
    assert!(manifest_props.contains_key("ingredientDeltas"), "manifest must define ingredientDeltas (per-manifest)");
...
fn test_validation_results_definition_and_ingredient_usage() -> Result<()> {
    ...
    assert!(
        vr_required.iter().any(|v| v.as_str() == Some("activeManifest")),
