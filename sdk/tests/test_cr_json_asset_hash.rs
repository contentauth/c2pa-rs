...
//! Integration tests for CrJsonReader asset hash functionality
...
#[test]
fn test_complete_cr_json_format_with_asset_info() -> Result<()> {
    let mut reader = CrJsonReader::from_stream("image/jpeg", Cursor::new(IMAGE_WITH_MANIFEST))?;
    ...
    let asset_info = json_value["asset_info"]
        .as_object()
        .expect("asset_info should be an object");
