...
//! Schema compliance tests for JPEG Trust format
...
fn test_validation_status_schema_compliance() -> Result<()> {
    ...
    let error = validation_status.get("error").expect("error field should exist");
    assert!(
...
fn test_manifest_status_schema_compliance() -> Result<()> {
    let reader = JpegTrustReader::from_stream("image/jpeg", Cursor::new(IMAGE_WITH_MANIFEST))?;
    let json_value = reader.to_json_value()?;
        ...
        if let Some(status) = manifest.get("status") {
            ...
            if let Some(assertion) = status_obj.get("assertion") {
                ...
                for (_key, value) in assertion.as_object().unwrap() {
                    assert!(value.is_string(), "assertion status values should be strings");
                }
            }
        }
    }
...
fn test_complete_schema_structure() -> Result<()> {
    ...
    assert!(json_value.get("asset_info").is_some(), "asset_info missing (with hash)");
